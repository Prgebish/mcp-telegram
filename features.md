...
### Гранулярные права
**Description:** Каждому чату можно назначить комбинацию из трёх прав.
...
Один глобальный token bucket. Все запросы к Telegram API проходят через `limiter.Wait(ctx)` перед выполнением.
